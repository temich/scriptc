...
- `URL` supports construction from one absolute string and read-only `protocol`, `origin`, `username`, `pathname`, `href`, `host`, `hostname`, and `search` getters. `searchParams` remains a live view. Relative inputs with a base argument and setters remain fenced.
